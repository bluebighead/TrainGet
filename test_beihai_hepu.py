...
def test_beihai_hepu():
    """
    测试北海到合浦的车票查询
    """
    logger.info("开始测试北海到合浦的车票查询...")
    ...
    try:
        ...
        logger.info(f"获取站点编码: {start_station} -> {from_station}, {end_station} -> {to_station}")
        ...
        # 保存响应内容到文件
        with open('debug_beihai_hepu.json', 'w', encoding='utf-8') as f:
            f.write(response.text)
        logger.info("响应内容已保存到 debug_beihai_hepu.json")
