...
def debug_query():
    ...
    try:
        ...
        # 保存响应内容到文件
        debug_file = "debug_advanced_anti_crawl.html"
        ...
        try:
            ...
            # 尝试使用不同的URL格式
            logger.info("尝试使用不同的URL格式...")
            alt_query_url = f"https://kyfw.12306.cn/otn/leftTicket/query?leftTicketDTO.train_date=2026-02-20&leftTicketDTO.from_station=BHZ&leftTicketDTO.to_station=GLZ&purpose_codes=ADULT"
            
            # 随机等待一段时间
            time.sleep(random.uniform(2, 4))
            
            alt_response = session.get(alt_query_url, headers=query_headers)
            logger.info(f"备选查询请求成功，状态码: {alt_response.status_code}")
            
            # 保存备选响应内容到文件
            alt_debug_file = "debug_advanced_anti_crawl_alt.html"
            with open(alt_debug_file, 'w', encoding='utf-8') as f:
                f.write(alt_response.text)
            logger.info(f"备选响应内容已保存到 {alt_debug_file}")
            
            # 尝试解析备选响应
            try:
                alt_result = alt_response.json()
                logger.info(f"备选JSON解析成功")
                logger.info(f"备选响应状态: {alt_result.get('status')}")
            except json.JSONDecodeError as alt_e:
                logger.error(f"备选JSON解析失败: {alt_e}")
